...
# ---------------- Expanded Gate ---------------- #
class ExpandedGate(nn.Module):
    def __init__(self, base_fn="silu"):
        """
        base_fn: 'silu', 'gelu', or 'arctan'
        """
        super().__init__()
        self.alpha = nn.Parameter(torch.tensor(0.0))  # trainable scalar
        self.base_fn = base_fn.lower()
        
    def forward(self, x):
        if self.base_fn == "silu":
            gate = torch.sigmoid(x)
        elif self.base_fn == "gelu":
            gate = 0.5 * (1.0 + torch.erf(x / torch.sqrt(torch.tensor(2.0, device=x.device))))
        elif self.base_fn == "arctan":
            gate = 0.5 + torch.atan(x) / torch.pi
        else:
            raise ValueError(f"Unknown base function {self.base_fn}")
        
        # Expand gate range: (-α, 1+α)
        gate_expanded = (1 + 2 * self.alpha) * gate - self.alpha
...
class Vgg19(nn.Module):
    def __init__(self, act_fun="relu", num_classes=10):
        super().__init__()
        ...
        self.classifier = nn.Sequential(
            nn.Linear(512 * 1 * 1, 4096), nn.ReLU(inplace=False), nn.Dropout(0.5),
            nn.Linear(4096, 4096), nn.ReLU(inplace=False), nn.Dropout(0.5),
    ...
    def _get_act_fun(self, act_fun):
        ...
        if act_fun == "xgelu": return ExpandedGate("gelu")
        if act_fun == "xsilu": return ExpandedGate("silu")
        if act_fun == "xatlu": return ExpandedGate("arctan")
    ...
    def forward(self, x):
        for layer in self.features:
            if isinstance(layer, nn.ReLU):
                x = self.act_fun(x) if not isinstance(self.act_fun, F.relu) else F.relu(x)
            else:
                x = layer(x)
        x = torch.flatten(x, 1)
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.conv1(x)
        out = self.bn1(out)
        out = self.act_fun(out) if not isinstance(self.act_fun, F.relu) else F.relu(out)
...
def ResNet18(act_fun="relu", num_classes=10):
    return ResNet(BasicBlock, [2, 2, 2, 2], act_fun=act_fun, num_classes=num_classes)
